<?php

namespace App\Http\Controllers\Auth;

use App\Http\Controllers\Controller;
use App\Http\Requests\Auth\LoginRequest;
use App\Providers\RouteServiceProvider;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;

class AuthenticatedSessionController extends Controller
{
    /**
     * Display the login view.
     *
     * @return \Illuminate\View\View
     */
    public function create()
    {
        return view('auth.login');
    }

    /**
     * Handle an incoming authentication request.
     *
     * @param  \App\Http\Requests\Auth\LoginRequest  $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function store(LoginRequest $request)
    {
        $request->authenticate();

        $request->session()->regenerate();

        if(Auth::user()->user_type == 'administrator'){

            return redirect()->intended(RouteServiceProvider::ADMINHOME);

        }

        if(Auth::user()->status == 'enabled'){

             if(Auth::user()->category == 'Entrepreneur'){

                 return redirect()->intended(RouteServiceProvider::ENTERPRENEURHOME);

             }

            if(Auth::user()->category == 'Employer'){

                 return redirect()->intended(RouteServiceProvider::EMPLOYERHOME);

            }

             if(Auth::user()->category == 'Investor'){

                 return redirect()->intended(RouteServiceProvider::INVESTORHOME);

             }

             return redirect()->intended(RouteServiceProvider::HOME);

        }
        elseif(Auth::user()->status == 'disabled'){

            return redirect('/disabled_account') ;

        }

    }

    /**
     * Destroy an authenticated session.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function destroy(Request $request)
    {
        Auth::guard('web')->logout();

        $request->session()->invalidate();

        $request->session()->regenerateToken();

        return redirect('/');
    }
}
