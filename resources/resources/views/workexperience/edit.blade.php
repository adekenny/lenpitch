...
<!-- overlay -->
<div id="modal_overlay" class="hidden absolute inset-0 bg-black bg-opacity-80 h-screen w-full flex justify-center items-start md:items-center pt-5 md:pt-0">

<!-- modal -->
<div id="modal" class="pacity-0 transform -translate-y-full scale-150  relative w-10/12 md:w-2/3 h-2/3 md:h-5/6 bg-white rounded shadow-lg duration-300">
    ...
    <form action="{{ url('workexperience/'.$workexperience->id) }}" method="POST">
                        @csrf
                        @method('PATCH')
            <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-1 pb-4">
                <div>
                    <x-label for="title" :value="__('Job Title:')" />
                    <x-input id="title" class="block mt-1 w-full" type="text" name="title" value="{{$workexperience->title}}" required autofocus />
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                 ...
                 <div>
                    ...
                    <x-input id="company_name" class="block mt-1 w-full" type="text" name="company_name" value="{{$workexperience->company_name}}" required autofocus />
                </div>

                <div>
                    <x-label for="employment_type" :value="__('Employment Type:')" />
                    <x-input id="employment_type" class="block mt-1 w-full" type="text" name="employment_type" value="{{$workexperience->employment_type}}" required autofocus/>
                </div>

                <div>
                    <x-label for="location" :value="__('Location:')" />
                    <x-input id="location" class="block mt-1 w-full" type="text" name="location" value="{{$workexperience->location}}" required autofocus />
                </div>

                <div>
                <x-label for="start_date" :value="__('Start Date:')" />
                    <x-input id="start_date" class="block mt-1 w-full" type="text" name="start_date" value="{{$workexperience->start_date}}" required autofocus />
                </div>

                <div>
                <x-label for="end_date" :value="__('End Date:')" />
                    <x-input id="end_date" class="block mt-1 w-full" type="text" name="end_date" value="{{$workexperience->end_date}}" required autofocus />
                </div>

                <div>
                <x-label for="industry" :value="__('Industry:')" />
                    <x-input id="industry" class="block mt-1 w-full" type="text" name="industry" value="{{$workexperience->industry}}" required autofocus />
                </div>
                
            </div>
            <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-1 pb-4">
                <div>
                    <x-label for="description" :value="__('Job Descriptions:')" />
                    <textarea id="description" cols="5" rows="2"  class="block mt-1 w-full" name="description">{{$workexperience->description}}</textarea>
                </div>
            </div>

            <div class="flex justify-between mt-6">
               
               <button name="update" class="px-6 py-2 leading-5 text-white transition-colors duration-200 transform bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600">Update</button>
               <button name="delete" class="px-6 py-2 leading-5 text-white transition-colors duration-200 transform bg-red-700 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600">Delete</button>               
        </div>
