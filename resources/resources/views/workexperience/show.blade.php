...
<!-- overlay -->
<div id="modal_overlay" class="hidden absolute inset-0 bg-black bg-opacity-30 h-screen w-full flex justify-center items-start md:items-center pt-2 md:pt-0">

<!-- modal -->
<div id="modal" class="pacity-0 transform -translate-y-full scale-150  relative w-10/12 md:w-2/3 h-1/2 md:h-3/4 bg-white rounded shadow-lg duration-300">
    ...
    <div class="px-4 py-3 border-b border-gray-200 h-auto">
        <h2 class="text-2xl">Work Experience</h2>  
    </div>
    <div class="grid grid-cols-2 gap-8 px-4 py-4">
            <div class="text-lg col-span-2"><b>Job Title</b>: {{ $workexperience->title}}</div>
            <div class="text-lg"><b>Company Name</b>: {{ $workexperience->company_name}}</div><div class="text-lg"><b>Employment Type</b>: {{$workexperience->employment_type}}</div>
           
            <div class="text-lg"><b>Location</b>: {{ $workexperience->location}}</div> <div class="text-lg"><b>Start Date</b>: {{ $workexperience->start_date}}</div>
            <div class="text-lg"><b>End Date</b>: {{ $workexperience->end_date}}</div><div class="text-lg"><b>Industry</b>: {{ $workexperience->industry}}</div>
            <div class="text-lg col-span-2"><b>Job Description</b>: {{ $workexperience->description}}</div>
            
    </div>
    <br/><br/>
    <div class="flex justify-between px-6">
    <div>  <a href="{{ url('workexperience/'.$workexperience->id.'/edit') }}"><button name="update" class="px-6 py-2 leading-5 text-white transition-colors duration-200 transform bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600">Edit</button></a></div>
    <div><form method="post" action="{{route('workexperience.destroy',$workexperience->id)}}">
                            @method('delete')
